...
| **3** | **Territory: Feeder Wars** | 4K YOLO `track_bboxes` + `track_frames` | Python + React/SVG | Moderate | Ecological turf-war strategy |
...
4. **Phase 4 (Deep Gameplay): Project 3 (*Feeder Wars*) & Project 2 (*Feederbound*)**
   * **Feeder Wars** — built, on the Wars tab. **Feederbound** — not started; it is the remaining half of this phase and the largest single build in the document.
   * *As built:* `/api/territory` plus `TerritoryMap.tsx`. The four zones are the four places birds really land, read off the yard's own density heatmap rather than invented as quadrants: the hanging cage, the dish, the drop zone and the rail. The map is SVG over the same camera still the heatmap is drawn on. Seven factions, because the roadmap's four barely intersect this yard: doves and pigeons are the two most-logged birds on the camera and needed the Columbid Order, the cardinal is top-five on its own, and the hawks and Wood Ducks that pass through needed the Outriders.
   * *The data gap this phase had to close:* `track_bboxes` stored no frame index, so two tracks in one clip could not be put on a shared clock and a displacement was **not computable at all** on any stored row. `Detection.track_frames` was added here and is written from the tracker's own `frame_index`. It is NULL on the entire archive, so old visits can be scored for occupancy and can never yield a pecking order, and footage from now on can. The page reports how many of a day's birds carry a clock rather than letting an empty log read as a peaceful yard.
   * *Control is scored in landings until timings cover half a day.* With only a few timed rows, seconds are worse than useless: every untimed bird scores zero, so a perch with two real landings reads as unheld while one timed visitor takes the whole day. That was a live bug during the build and now has a regression test.
   * *Open question:* the displacement rule is the design document's "within 1.0 s" made precise, which at 3 fps is three frames. Whether three frames is the right window is a guess. Nothing in the archive can settle it, so it wants a week of timed footage and then a look at what it caught and what it called sharing.
   * *Also open:* the built-in zone map is a claim about where this specific feeder stands, and it stops being true the day the shepherd's hook moves. A `zones.json` generated by clustering real perch points would override it, and nothing generates one yet.
